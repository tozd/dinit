...
func TestReparentingAdoptCancel(t *testing.T) {
	...
	assertLogs(t, []string{
		...
		`.+Z dinit: info: sleep/\d+: terminating`,
		`.+Z dinit: info: sleep/\d+: sending SIGTERM to PID \d+`,
		`.+Z dinit: info: sleep/\d+: PID \d+ finished with signal 15`,
	}, l)
}
...
func TestRunServices(t *testing.T) {
	...
	stderr := withLogger(t, log.Default(), func() {
		stdout = withLogger(t, dinit.StdOutLog, func() {
			...
			require.NoError(t, e)
			e = os.WriteFile(path.Join(dir, "service2", "terminate"), []byte("#!/bin/sh\necho end\n>&2 echo enderr\nkill -TERM $DINIT_PID"), 0o755) //nolint:gosec
			require.NoError(t, e)
	...
	assertLogs(t, []string{
		...
		`.+Z dinit: info: service2/run: terminating`,
		`.+Z dinit: info: service3/run: terminating`,
		...
		`.+Z dinit: info: service2/term: running with PID \d+`,
		`.+Z service2/term: enderr`,
		`.+Z dinit: info: service2/term: PID \d+ finished with status 0`,
